...
extension RoamSwitchKnowledgeBase {
    ...
    private static func featuresJaNetwork() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                id: "feat_nmap_nse",
                title: "nmap NSE 補完スキャン（実証型脆弱性診断の追加レイヤー） — 既定オフ",
                summary: "「実証型脆弱性診断」が有効な場合に限り、システムにインストール済みのnmapを使って露出ポートへ「safe」カテゴリのNSEスクリプトを追加実行し、この製品自前のプローブが持たないプロトコルカバレッジ（SSH鍵情報、SMTP応答等）を補う。判定はnmap自身のものであり、本製品による独立検証ではない。",
                details: """
                • 有効化: 「実証型脆弱性診断（能動的到達確認）」を先にオンにした上で、「🔎 nmap NSE 補完スキャン」を追加でオンにする。2段階のオプトインで、既存の実証型脆弱性診断とは独立した設定。nmapは自動インストールされず、システムに既にインストール済み（Homebrew等）の場合のみ機能する（未インストール時は何もしない）。
                • スクリプト選定: `safe and not broadcast and not external`。単純な`safe`カテゴリだけでは不十分——`broadcast`系スクリプトは対象ホストだけでなくLAN全体へブロードキャスト/マルチキャストで問い合わせてしまい、`external`系スクリプト（`vulners.nse`等）は検出したサービス/バージョンをvulners.com等のサードパーティへ実際に送信する。いずれも「127.0.0.1のみ・他ホストや外部には一切触れない」という本製品の設計方針に反するため除外している。
                • タイムアウト: スクリプト単位で15秒（`--script-timeout 15s`）。一部の`safe`スクリプトが非標準的なHTTP API等に対して際限なく時間を消費し、他ポートの結果まで失われることがあるための対策。
                • 対象範囲: 「実証型脆弱性診断」の対象と同じ、既に開放が確認されたポートのみ。
                """,
                recommendation: "nmapの所見はあくまで参考情報として扱い、内容を確認した上で該当する場合は対応してください。"
            ),
            LocalizedEntry(
    ...
    private static func featuresJaAudit() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                • 通知内容: 異常の件数（新規パターン・頻度急増の内訳）、最大3件の実際のログ行、学習状況の注記、非専門家向けの説明。頻度急増を含む場合は通知センターに表示しますが、新規パターンのみの場合はポップアップを出さず通知履歴にのみ記録します。新規パターンは一度記録すると「既知」として扱われ、同じ内容で再度記録されません。頻度急増はパターンごとの学習が完了すると鳴らなくなります。
            ...
            LocalizedEntry(
                ...
                title: "着信ポートスキャン検知 (自動遮断, Pro)",
                summary: "短時間(5分間)に多数の異なるポート(15件以上)へ接続してきた送信元IPを検知して通知します(nmap/masscan等の偵察行為の典型的な特徴)。検知したスキャン元は既定で10分間、自動的に遮断されます。Pro版限定。",
                details: """
                • 仕組み: 特権ヘルパーが`tcpdump -i pflog0`でpf(パケットフィルタ)のログを監視し、短時間ウィンドウ内で異なる宛先ポートに一定件数以上到達した送信元IPを検知する。判定はログ記録のみに基づき、通信内容そのものを変更・検査することはない。Linux版の`port_scan_detect.rs`(nftables `log` + `journalctl`)に相当する機能。
                • 自動遮断: 検知したスキャン元は既定で10分間、`PFRulesetCoordinator`経由でpfルールに追加され遮断される。自動遮断のオン・オフは検知機能自体のオン・オフとは独立して切り替えられる。
                • 通知: 検知(および遮断)のたびにmacOS通知を送信し、統合インシデントタイムラインにも記録される。
                • 開き方: メニューバーの「ポート・デバイス監視」→「🔍 着信ポートスキャン検知 (Pro)」。有効化・無効化どちらも確認ダイアログを経由する。初期設定オフ。
                """,
                recommendation: "IPごとの許可リストはなく、遮断は10分で自動的に解除されます。自宅や社内で正規のスキャンツール(資産管理・脆弱性診断等)を定期的に走らせている場合、誤検知で繰り返し遮断される可能性があるため、その間は自動遮断だけをオフにして検知(通知)のみ有効にしておくことを検討してください。"
            ),
            LocalizedEntry(
                id: "feat_sensor_pairing",
                title: "RoamSwitch Sensor ペアリング (mDNS相互信頼, Pro)",
                summary: "同一LAN上に置く別製品「RoamSwitch Sensor」(専用の能動監査ハブ)との相互信頼ペアリングを管理します。mDNSで自身を通知しつつSensorを探索しますが、発見しただけでは信頼せず、操作者の明示的な「ペアリング」操作が必要です(Bluetoothペアリングと同じ考え方)。Pro版限定。",
                details: """
                • 仕組み: この端末自身のEd25519鍵ペアを生成・永続化し、公開鍵をmDNS(サービスタイプ`_roamswitch._tcp`)のTXTレコードで`role=endpoint`として広告する。同時にSensor側の`role=sensor`広告を探索する。Sensor側がこの端末を信頼するには、Sensor側のペアリング機能にこの端末の公開鍵を別途教える必要がある(一方的な発見だけでは双方向の信頼は成立しない)。
                • 手動ペアリング: 一部のWi-Fiアクセスポイントではmulticastが非対称に転送され、mDNS相互探索が信頼できないことがあるため、Sensor側に表示される公開鍵・アドレスを直接入力する手動ペアリングも常に使える(mDNSが機能しなくてもペアリング自体は失敗しない設計)。
                • 有効化の影響: オンにするとこの端末がmDNSで自身の存在(ホスト名・公開鍵)をLAN上に通知するようになる。オフ(初期設定)ならこの通知は発生しない。
                • 開き方: メニューバーの「ポート・デバイス監視」→「🔍 RoamSwitch Sensor ペアリング…」。この端末自身の公開鍵/アドレス表示(コピーボタン付き)、発見されたSensor一覧(ペアリングボタン)、ペアリング済みSensor一覧(解除ボタン)、手動ペアリングフォームを持つ。
                • Linux版(`roamswitch-core::sensor_pairing`)とワイヤープロトコル(サービスタイプ・TXTレコードのキー/値)を完全に一致させている。
                """,
                recommendation: "ペアリングは、実際に自分が設置したRoamSwitch Sensorであることを確認してから行ってください。見慣れないSensorが発見された場合はペアリングせず、ネットワーク上の管理者に確認することを推奨します。"
            ),
            LocalizedEntry(
