...
extension RoamSwitchKnowledgeBase {
    ...
    private static func featuresJaNetwork() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                id: "feat_nmap_nse",
                title: "nmap NSE 補完スキャン（実証型脆弱性診断の追加レイヤー）",
                ...
                details: """
                • 自動実行: 「実証型脆弱性診断（能動的到達確認）」が有効であれば自動的に実行される。個別のオン/オフ設定はない。nmapは自動インストールされず、システムに既にインストール済み（Homebrew等）の場合のみ機能する（未インストール時は何もしない）。
    ...
    private static func featuresJaAudit() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                title: "RoamSwitch Sensor ペアリング (ペアリングコード方式, Pro)",
                summary: "同一LAN上に置く別製品「RoamSwitch Sensor」(専用の能動監査ハブ)との相互信頼ペアリングを管理します。Sensor側で発行されるペアリングコードを使った能動ペアリング方式で、mDNSによる自動探索は使用しません(固定IPで運用されるSensorが前提)。Pro版限定。",
                details: """
                • 仕組み: この端末自身のEd25519鍵ペアを生成・永続化する。ペアリング時はSensor側オペレーターが発行した使い捨てのペアリングコード(発行から10分で失効)と、この端末のアドレス・ホスト名を添えてSensorのTCPリスナー(ポート50543)へ接続する。コードが有効であれば、Sensorはこの端末の公開鍵を信頼済み一覧に登録する。
            ...
            LocalizedEntry(
                ...
                """,
                recommendation: "初回起動時の案内に従ってヘルパーを承認してください。承認されていない場合、メニューに「⚠️ ヘルパーを承認する…」が表示されます。アップデート後にこの表示や通知が出た場合も、同じ手順（「システム設定」>「一般」>「ログイン項目とApp機能拡張」）で再承認できます。"
            ),
            LocalizedEntry(
                id: "feat_mcp_server",
