    ...
    private static func featuresPtPTNetwork() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                summary: "A contenção de emergência partilhada usada quando é detetada uma ameaça grave (ransomware, deteção de malware pelo XProtect, falsificação de ARP, ClickFix). Bloqueia todo o tráfego de entrada e de saída. Também corta as ligações que já estavam abertas. Nos disparadores de baixa confiança (como o ClickFix), a rede regressa automaticamente no máximo em 10 minutos, mesmo após uma falha ou reinício. Os disparadores de alta confiança (ficheiro isco de ransomware, deteção do XProtect, falsificação de ARP numa rede de Bloqueio máximo, contenção ARP iniciada com «Cortar toda a rede agora» a partir do aviso de falsificação) nunca voltam a abrir sozinhos: após até 1 hora de corte total passam para um modo degradado que continua a bloquear novas ligações de saída.",
                details: """
                ...
                • Libertação: libertar a partir da janela de emergência ou da notificação remove o bloqueio do pf e volta a ligar o Wi-Fi. O item da barra de menus «Levantar isolamento Air-Gap» está sempre disponível, também na versão gratuita e mesmo que uma licença Pro expire a meio do incidente. Os avisos sobre o estado do isolamento (iniciado, degradado, levantado, falhado) também chegam aos utilizadores gratuitos; os alertas de deteção continuam a ser Pro.
                • Rede de segurança: se ninguém a libertar (incluindo após uma falha da app ou um reinício do Mac), um temporizador do lado do auxiliar atua conforme a confiança do disparador. Confiança baixa (ClickFix, heurísticas genéricas, um Air-Gap manual simples sem aviso): o Air-Gap é libertado à força ao fim de 10 minutos e o Wi-Fi é restaurado. Confiança alta (ficheiro isco de ransomware, deteção do XProtect, falsificação de ARP numa rede de Bloqueio máximo, contenção ARP iniciada com «Cortar toda a rede agora» a partir do aviso de falsificação): Air-Gap total até 1 hora e depois um modo degradado. No modo degradado as novas ligações de saída (exceto DHCP) continuam bloqueadas, mas o Wi-Fi volta a ligar-se para poder ver em que rede está; nunca abre por temporizador, só quando o libertar ou uma nova verificação confirmar que a causa desapareceu (por exemplo, o processo suspeito terminou). A nova verificação começa cerca de 10 minutos após o corte total e, se a causa se confirmar desaparecida, levanta o isolamento sem esperar pelo limite de 1 hora (como na edição Linux); enquanto o rádio Wi-Fi está desligado não é possível ler a cache ARP, pelo que a verificação ocorre quando o modo reduzido volta a ligar o rádio. Na contenção ARP, a nova verificação lê a cache ARP (só enquanto falta a entrada do gateway, envia antes um pedido ARP na ligação, que o isolamento não bloqueia por não ser tráfego IP) e só considera a causa desaparecida se o MAC de confiança do gateway surgir em 3 verificações consecutivas ao longo de, pelo menos, 60 segundos (outro MAC, uma entrada em falta ou incompleta, um MAC duplicado ou uma cache ilegível recomeçam a contagem). As ligações já abertas são cortadas quando o Air-Gap é ativado e de novo ao passar para o modo degradado.
    ...
    private static func featuresPtPTMalware() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                title: "Registo de execução de processos (eslogger, apenas notificação, Pro, desativado por predefinição)",
                summary: "Regista os programas que arrancam neste Mac através do /usr/bin/eslogger da Apple (macOS 13+) e avisa quando um arranque corresponde a uma combinação suspeita. Nunca bloqueia uma execução nem corta a rede; o registo fica neste Mac.",
                details: """
                • Como funciona: o auxiliar privilegiado executa `/usr/bin/eslogger exec fork exit` como processo filho e analisa o seu fluxo JSON. O eslogger vem com o macOS; o RoamSwitch não usa nem pede o entitlement EndpointSecurity, pelo que se trata de observação a posteriori, não de bloqueio antes da execução.
                • Requisitos: macOS 13 ou posterior e Acesso total ao disco para o RoamSwitchHelper. Sem ele, a janela mostra «Registo de execução indisponível: o RoamSwitchHelper precisa de Acesso total ao disco» (ou que o eslogger não foi encontrado) e nada mais é feito; não há recurso a sondagem constante. Os eventos anteriores à ativação ou ao arranque do auxiliar não são registados.
                • Regras de correlação (apenas notificam, silenciosas por predefinição, cada uma com ID estável e técnica MITRE ATT&CK): shell ou interpretador de scripts iniciado diretamente por um navegador, Office ou aplicação de correio (exec.shell_from_app, T1059); binário não assinado ou com assinatura ad hoc executado a partir de /tmp, /private/var/tmp ou com o atributo de quarentena (exec.untrusted_location, T1204.002); one-liner `sh -c` que passa curl/wget para uma shell com um fator agravante como URL com IP direto, base64, eval, verificação TLS desativada ou navegador como processo pai (exec.pipe_to_shell, T1059.004); `osascript -e` que combina do shell script com base64/eval (exec.osascript_obfuscated, T1059.002); `xattr -d com.apple.quarantine` seguido da execução desse ficheiro em 15 minutos (exec.quarantine_stripped_then_exec, T1553.001); binário não assinado ou ad hoc iniciado pelo launchd a partir de um LaunchAgent/LaunchDaemon escrito nas últimas 24 horas (exec.launchd_untrusted_binary, T1543.001/.004); `security find-generic-password -w` ou `dump-keychain` sob um ancestral que não é uma shell nem está assinado pela Apple (exec.keychain_access, T1555.001). Um simples `curl | sh` escrito no Terminal não é assinalado de propósito.
                ...
                """,
                recommendation: "Ative-o se quiser um histórico de execuções para análise de incidentes e conceda primeiro Acesso total ao disco ao RoamSwitchHelper. Trate os alertas como pistas a verificar no registo, não como prova de comprometimento."
            ),
            LocalizedEntry(
    ...
    private static func featuresPtPTAudit() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                • Proteção do transporte (TLS): a API de controlo é acedida por TLS 1.3 e o certificado do Sensor só é considerado fiável através da fixação (pinning) da sua impressão digital (SHA-256 do certificado; sem validação de AC nem de nome de anfitrião). No emparelhamento, introduz manualmente a impressão digital apresentada no ecrã do próprio Sensor (nunca é obtida pela rede). O pedido de emparelhamento fica associado por assinatura a essa impressão digital e à chave deste dispositivo, pelo que um intermediário com outro certificado falha. Se o Sensor regenerar o certificado (tls rotate), as ligações são recusadas; volte a fixar com «Registar / atualizar impressão digital» usando o novo valor do ecrã do Sensor (nunca automático). Um Sensor emparelhado antes da fixação existir continua a usar texto simples (com um aviso na janela); se o TLS falhar, não há recurso automático a texto simples.
                """,
            ...
            LocalizedEntry(
                ...
                • Nota: os testes de Air-Gap cortam mesmo a rede temporariamente. Liberte a partir da janela de emergência (as simulações contam como confiança alta, por isso não se libertam sozinhas ao fim de 10 minutos; após até 1 hora passam para o modo degradado).
            ...
            LocalizedEntry(
                ...
                • Daemons associados: também são registados LaunchDaemons auxiliares para a rede de segurança do Air-Gap (libertação automática ao fim de 10 minutos com confiança baixa, modo degradado após o limite com confiança alta) e a porta de arranque (até 90 segundos).
            ...
            LocalizedEntry(
                ...
                • Registo de execução de processos (Pro, só de leitura): `search_exec_events` (pesquisar eventos de arranque) e `get_process_tree` (ancestrais e descendentes de um processo).
    ...
    private static func alertsPtPTMalware() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                • Defesa automática: todo o tráfego cortado, Wi-Fi desligado e ligações já abertas cortadas. Uma deteção do XProtect é de confiança alta, pelo que não é restaurada ao fim de 10 minutos: após até 1 hora de corte total passa para um modo degradado (sem novas ligações de saída) até o libertar ou uma nova verificação confirmar que a causa desapareceu. São registados o processo detetor, a categoria e a mensagem de deteção da Apple.
                """,
                recommendation: """
    ...
    private static func troubleshootingPtPTOperation() -> [LocalizedEntry] {
        return [
            LocalizedEntry(
                ...
                • Restauro automático: nos disparadores de baixa confiança (como o ClickFix), a rede de segurança do auxiliar restaura a rede em 10 minutos mesmo sem libertar. Nos de alta confiança (ficheiro isco de ransomware, deteção do XProtect) a rede não volta a abrir sozinha: após até 1 hora passa para o modo degradado até a libertar a partir da janela de emergência ou da notificação, ou uma nova verificação confirmar que a causa desapareceu. O mesmo se aplica após fechar a app, uma falha ou um reinício.
