    ...
    private static func featuresZhHantNetwork() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                • 保護機制：若一直沒有人解除（包括應用程式當機或 Mac 重新開機之後），輔助工具端的計時器會依觸發的可信度處理。低可信度（ClickFix、一般啟發式偵測、沒有警告的一般手動氣隙）：10 分鐘後強制解除氣隙並恢復 Wi-Fi 無線電。高可信度（勒索軟體誘餌檔案、XProtect 判定、最大防護網路中的 ARP 詐騙、從 ARP 詐騙警告中按下「立即全部斷網」手動啟動的 ARP 圍堵）：完全氣隙最長 1 小時，之後轉入降級模式。降級模式下新的對外連線（DHCP 除外）仍被阻擋，但會重新開啟 Wi-Fi 無線電，方便您查看所連的網路；它不會因時間到了而自動放行，只有您手動解除，或再次檢查確認原因已消除（例如可疑程序已結束）時才會解除。再次檢查大約從完全阻斷 10 分鐘後開始，確認原因已消除時，無需等待 1 小時上限即可解除（與 Linux 版相同）；Wi-Fi 無線關閉期間無法讀取 ARP 快取，因此要等降級模式恢復無線後才會檢查。對於 ARP 圍堵，再次檢查會讀取 ARP 快取（僅在閘道項目遺失時，先在鏈路上傳送 1 次 ARP 請求；它不是 IP 流量，不會被隔離攔截），只有在至少 60 秒內連續 3 次都看到閘道的可信 MAC 時，才認定原因已消除（MAC 不符、項目遺失或未完成、MAC 重複、無法讀取都會重新計數）。氣隙啟動時以及轉入降級模式時，已建立的連線都會被切斷。
    ...
    private static func featuresZhHantMalware() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                title: "勒索軟體復原（從事前快照中逐檔取出）",
                summary: "為了回到加密之前的狀態，RoamSwitch 會定期建立 APFS 本機快照，只把需要的檔案取出並複製到其他位置。不會覆蓋目前的檔案。",
                details: """
                • 事前快照：與偵測無關，預設每 6 小時建立一次（可選：關閉/1/3/6/12/24 小時）。偵測之後建立的快照，可能已經包含被加密的檔案。
                ...
                """,
                recommendation: "為防範勒索軟體，請保持事前快照開啟。發現受損後，請先中斷網路，再從「建議」的事前快照（而非偵測時快照）中取出檔案。"
            ),
            LocalizedEntry(
