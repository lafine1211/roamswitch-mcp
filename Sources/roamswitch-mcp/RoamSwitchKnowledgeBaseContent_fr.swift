    ...
    private static func featuresFrNetwork() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                summary: "Le confinement d'urgence partagé utilisé lorsqu'une menace grave est détectée (rançongiciel, détection de malware par XProtect, usurpation ARP, ClickFix). Il bloque tout le trafic entrant et sortant. Il coupe aussi les connexions déjà ouvertes. Pour les déclencheurs de faible confiance (comme ClickFix), le réseau revient automatiquement en 10 minutes maximum, même après un plantage ou un redémarrage. Les déclencheurs de forte confiance (leurre de rançongiciel, détection XProtect, usurpation ARP sur un réseau en Verrouillage maximal, confinement ARP lancé avec « Couper tout le réseau maintenant » depuis l'avertissement d'usurpation) ne se rouvrent jamais d'eux-mêmes : après 1 heure au plus de coupure totale, ils passent en mode dégradé, qui continue de bloquer les nouvelles connexions sortantes.",
                details: """
                ...
                • Libération : lever le confinement depuis la fenêtre d'urgence ou la notification retire le blocage pf et réactive le Wi-Fi. L'élément de la barre des menus « Lever l'isolement Air-Gap » est toujours disponible, y compris en version gratuite et même si une licence Pro expire en plein incident. Les notifications sur l'état de l'isolation (démarrée, dégradée, levée, échouée) parviennent aussi aux utilisateurs gratuits ; les alertes de détection restent Pro.
                • Filet de sécurité : si personne ne le lève (y compris après un plantage de l'app ou un redémarrage du Mac), une minuterie côté assistant agit selon la confiance du déclencheur. Faible confiance (ClickFix, heuristiques génériques, un Air-Gap manuel simple sans avertissement) : l'Air-Gap est levé de force après 10 minutes et le Wi-Fi est rétabli. Forte confiance (leurre de rançongiciel, détection XProtect, usurpation ARP sur un réseau en Verrouillage maximal, confinement ARP lancé avec « Couper tout le réseau maintenant » depuis l'avertissement d'usurpation) : Air-Gap total pendant 1 heure au plus, puis mode dégradé. En mode dégradé, les nouvelles connexions sortantes (hors DHCP) restent bloquées, mais le Wi-Fi est réactivé pour que vous voyiez sur quel réseau vous êtes ; il ne s'ouvre jamais sur minuterie, seulement lorsque vous le levez ou qu'une nouvelle vérification confirme que la cause a disparu (par exemple le processus suspect s'est terminé). La nouvelle vérification commence environ 10 minutes après la coupure totale et, si la disparition de la cause est vérifiée, lève l'isolement sans attendre la limite de 1 heure (comme l'édition Linux) ; tant que la radio Wi-Fi est coupée, le cache ARP est illisible, donc la vérification a lieu une fois que le mode dégradé rallume la radio. Pour le confinement ARP, la nouvelle vérification lit le cache ARP (seulement tant que l'entrée de la passerelle manque, elle envoie d'abord une requête ARP sur le lien, que l'isolement ne bloque pas car ce n'est pas du trafic IP) et ne considère la cause comme disparue que si l'adresse MAC de confiance de la passerelle apparaît lors de 3 vérifications consécutives couvrant au moins 60 secondes (une autre MAC, une entrée absente ou incomplète, une MAC en double ou un cache illisible font repartir de zéro). Les connexions déjà ouvertes sont coupées à l'activation de l'Air-Gap puis à nouveau au passage en mode dégradé.
            ...
            LocalizedEntry(
                id: "feat_nmap_nse",
                title: "Analyse complémentaire nmap NSE (couche supplémentaire pour la vérification active des vulnérabilités)",
                ...
                details: """
                • Inclus automatiquement : s'exécute automatiquement dès que « Vérification active des vulnérabilités (vérification active de l'accessibilité) » est activée ; il n'existe pas de réglage séparé pour cela. nmap n'est jamais installé automatiquement — cela ne prend effet que s'il est déjà installé sur le système (par ex. via Homebrew) ; sinon, rien ne se passe.
    ...
    private static func featuresFrMalware() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                title: "Récupération après ransomware (extraction de fichiers depuis un instantané préventif)",
                summary: "Pour revenir à l'état d'avant tout chiffrement, RoamSwitch crée régulièrement des instantanés locaux APFS et vous permet d'extraire seulement les fichiers nécessaires vers un autre emplacement. Vos fichiers actuels ne sont jamais écrasés.",
                details: """
                • Instantanés préventifs : créés par défaut toutes les 6 heures (désactivé / 1 / 3 / 6 / 12 / 24 heures), indépendamment de toute détection. Un instantané pris après une détection peut déjà contenir des fichiers chiffrés.
                • Instantanés de détection : également créés quand la garde des leurres se déclenche, mais ils peuvent contenir des fichiers chiffrés et ne sont pas recommandés comme source de récupération.
                • Mode de conservation : tant qu'un instantané de détection est le plus récent (jusqu'à 7 jours), les nouveaux instantanés préventifs et le nettoyage des anciens sont suspendus, afin que la dernière génération d'avant le chiffrement ne soit pas évincée.
                • Extraction : via « Récupération après ransomware… » dans le menu, les fichiers ou dossiers de l'instantané recommandé (le plus récent instantané préventif encore présent) sont copiés vers `~/RoamSwitch-Recovered/<ID de l'instantané>/`. Les fichiers existants ne sont jamais écrasés et aucune restauration complète du volume n'est proposée. La récupération est toujours manuelle.
                • Limites : macOS peut supprimer lui-même les instantanés locaux après environ 24 heures (plus tôt si l'espace libre est faible) ; une génération disparue est inutilisable. L'extraction exige l'Accès complet au disque pour l'assistant RoamSwitch (pas la création).
                • MCP : l'outil en lecture seule `get_ransomware_recovery_snapshots` affiche la liste et la recommandation (il ne peut rien restaurer).
                • Pro: Cette fenêtre (liste et extraction de fichiers) et l'outil MCP sont des fonctions Pro. La création des instantanés fonctionne aussi sans Pro.
                """,
                recommendation: "Gardez les instantanés préventifs activés. En cas de dégât, coupez d'abord le réseau, puis extrayez les fichiers de l'instantané préventif recommandé, pas de celui de détection."
            ),
            LocalizedEntry(
            ...
            LocalizedEntry(
                ...
                title: "Enregistrement des exécutions de processus (eslogger, notification seule, Pro, désactivé par défaut)",
                summary: "Enregistre les programmes qui démarrent sur ce Mac grâce à /usr/bin/eslogger d'Apple (macOS 13+) et vous avertit lorsqu'un lancement correspond à une combinaison suspecte. Il ne bloque jamais une exécution et ne coupe jamais le réseau ; le journal reste sur ce Mac.",
                details: """
                • Fonctionnement : l'assistant privilégié lance `/usr/bin/eslogger exec fork exit` comme processus enfant et analyse son flux JSON. eslogger est fourni avec macOS ; RoamSwitch n'utilise ni ne demande l'entitlement EndpointSecurity, il s'agit donc d'une observation a posteriori, pas d'un blocage avant exécution.
                • Prérequis : macOS 13 ou ultérieur et Accès complet au disque pour RoamSwitchHelper. Sans lui, la fenêtre affiche « Enregistrement des exécutions indisponible : RoamSwitchHelper a besoin de l'Accès complet au disque » (ou qu'eslogger est introuvable) et rien d'autre n'est fait ; il n'y a pas de repli sur un sondage permanent. Les événements antérieurs à l'activation ou au démarrage de l'assistant ne sont pas enregistrés.
                • Règles de corrélation (notification seule, silencieuses par défaut, chacune avec un ID stable et une technique MITRE ATT&CK) : shell ou interpréteur de scripts lancé directement par un navigateur, Office ou une application de messagerie (exec.shell_from_app, T1059) ; binaire non signé ou à signature ad hoc exécuté depuis /tmp, /private/var/tmp ou portant l'attribut de quarantaine (exec.untrusted_location, T1204.002) ; commande `sh -c` qui transmet curl/wget à un shell avec un élément aggravant (URL à IP brute, base64, eval, vérification TLS désactivée ou navigateur parent) (exec.pipe_to_shell, T1059.004) ; `osascript -e` combinant do shell script et base64/eval (exec.osascript_obfuscated, T1059.002) ; `xattr -d com.apple.quarantine` suivi de l'exécution de ce fichier dans les 15 minutes (exec.quarantine_stripped_then_exec, T1553.001) ; binaire non signé ou ad hoc démarré par launchd depuis un LaunchAgent/LaunchDaemon écrit au cours des dernières 24 heures (exec.launchd_untrusted_binary, T1543.001/.004) ; `security find-generic-password -w` ou `dump-keychain` sous un ancêtre qui n'est pas un shell ni signé par Apple (exec.keychain_access, T1555.001). Un simple `curl | sh` saisi dans Terminal n'est volontairement pas signalé.
                ...
                """,
                recommendation: "Activez-le si vous voulez un historique des exécutions pour analyser les incidents, en accordant d'abord l'Accès complet au disque à RoamSwitchHelper. Considérez les alertes comme des pistes à vérifier dans le journal, pas comme la preuve d'une compromission."
            ),
            LocalizedEntry(
    ...
    private static func featuresFrAudit() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                title: "Vérification des signatures / de la provenance npm (npm audit signatures, facultatif, Pro)",
                summary: "Contacte le registre npm pour vérifier les signatures/la provenance des paquets installés. La seule fonction de RoamSwitch qui communique avec npmjs.com — désactivée par défaut, nécessite une activation explicite et une confirmation à chaque exécution. Pro uniquement.",
                details: """
                • Activation : le commutateur « Activer la vérification des signatures npm » dans l'onglet « 📦 Vérification CVE des paquets » → « Vérification des signatures npm (facultatif) (Pro) ». Cela ne fait que débloquer le bouton « Exécuter l'audit » pour chaque dossier de projet — rien n'est envoyé de son propre chef. Chaque exécution est confirmée par « Contacter le registre npm ? ».
            ...
            LocalizedEntry(
                ...
                title: "Appairage RoamSwitch Sensor (code d'appairage, Pro)",
                summary: "Gère l'appairage de confiance mutuelle avec un produit distinct, « RoamSwitch Sensor » (un concentrateur d'audit actif dédié), sur le même réseau local. Utilise un flux d'appairage actif via un code émis par le Sensor — aucune découverte automatique par mDNS, le Sensor étant supposé fonctionner avec une IP fixe. Pro uniquement.",
                details: """
                • Fonctionnement : génère et conserve de façon persistante sa propre paire de clés Ed25519. Pour s'appairer, cet appareil se connecte à l'écouteur TCP du Sensor (port 50543) avec un code d'appairage à usage unique émis par l'opérateur du Sensor (expire 10 minutes après émission), ainsi que l'adresse/le nom d'hôte de cet appareil. Si le code est valide, le Sensor ajoute la clé publique de cet appareil à sa liste de confiance.
                • Demander un audit : une fois appairé, « Demander un audit au Sensor » demande au Sensor d'exécuter un audit actif (vérification d'accessibilité). Le Sensor générant les résultats de façon asynchrone, l'assistant privilégié toujours actif de cet appareil interroge le résultat toutes les 5 minutes, jusqu'à 5 fois. Les résultats récupérés sont également enregistrés sur cet appareil et affichés sous « Résultats d'audit » dans l'écran des réglages.
                • Pour l'ouvrir : barre de menus → « Surveillance des ports et appareils » → « 🔍 Appairage RoamSwitch Sensor… ». Affiche la clé publique/l'adresse de cet appareil (avec un bouton de copie), les Sensors appairés (avec un bouton Désappairer), un formulaire de saisie du code d'appairage et la liste des résultats d'audit.
                • Utilise le même protocole de contrôle TCP que l'édition Linux (`roamswitch-core::sensor_pairing`) — port 50543, JSON délimité par des retours à la ligne, signatures Ed25519.
                • Protection du transport (TLS) : l’API de contrôle est jointe en TLS 1.3 et le certificat du Sensor n’est approuvé que par épinglage de son empreinte (SHA-256 du certificat ; ni autorité de certification ni nom d’hôte vérifiés). Lors de l’appairage, vous saisissez à la main l’empreinte affichée sur l’écran du Sensor (elle n’est jamais récupérée via le réseau). La demande d’appairage est liée par signature à cette empreinte et à la clé de cet appareil : un homme du milieu présentant un autre certificat échoue. Si le Sensor régénère son certificat (tls rotate), les connexions sont refusées ; réépinglez avec « Enregistrer / mettre à jour l’empreinte » en saisissant la nouvelle valeur affichée sur le Sensor (jamais automatique). Un Sensor appairé avant l’épinglage continue en texte clair (avertissement affiché) ; en cas d’échec TLS, il n’y a aucun repli automatique vers le texte clair.
                """,
                recommendation: "N'utilisez qu'un code d'appairage réellement émis depuis l'écran opérateur d'un Sensor que vous avez vous-même installé. Si l'on vous demande de saisir un code inconnu, n'appairez pas — renseignez-vous plutôt auprès de l'administrateur du réseau."
            ),
            LocalizedEntry(
            ...
            LocalizedEntry(
                ...
                • Remarque : les tests d'Air-Gap coupent réellement le réseau temporairement. Levez le confinement depuis la fenêtre d'urgence (les simulations comptent comme forte confiance : elles ne se lèvent pas d'elles-mêmes au bout de 10 minutes et passent en mode dégradé après 1 heure au plus).
            ...
            LocalizedEntry(
                ...
                • Démons associés : des LaunchDaemons assistants pour le filet de sécurité de l'Air-Gap (libération automatique après 10 minutes en faible confiance, mode dégradé après le plafond en forte confiance) et la porte de démarrage (jusqu'à 90 secondes) sont également enregistrés.
                ...
                • Nouvelle approbation après une mise à jour : l'app tente automatiquement de basculer vers le nouvel assistant, mais macOS peut tout de même le laisser en attente d'approbation. Dans ce cas, l'icône de la barre de menus passe en avertissement et affiche « ⚠️ Nouvelle approbation requise après la mise à jour », avec en plus une notification.
                """,
                recommendation: "Approuvez l'assistant lorsque vous y êtes invité au premier lancement. S'il n'est pas approuvé, le menu affiche « ⚠️ Approuver l'assistant… ». Si cet avertissement ou cette notification apparaît après une mise à jour, les mêmes étapes (Réglages Système > Général > Éléments d'ouverture et extensions) permettent de l'approuver à nouveau."
            ),
            LocalizedEntry(
                id: "feat_mcp_server",
                ...
                • Journal d'exécution des processus (Pro, lecture seule) : `search_exec_events` (rechercher les événements de lancement) et `get_process_tree` (ancêtres et descendants d'un processus).
    ...
    private static func alertsFrMalware() -> [LocalizedEntry] {
        return [
            ...
            LocalizedEntry(
                ...
                • Défense automatique : tout le trafic coupé, Wi-Fi désactivé et connexions déjà ouvertes coupées. Une détection XProtect est de forte confiance : elle n'est donc pas rétablie au bout de 10 minutes ; après 1 heure au plus de coupure totale, on passe en mode dégradé (aucune nouvelle connexion sortante) jusqu'à ce que vous le leviez ou qu'une nouvelle vérification confirme que la cause a disparu. Le processus détecteur, la catégorie et le message de détection d'Apple sont enregistrés.
                """,
                recommendation: """
    ...
    private static func troubleshootingFrOperation() -> [LocalizedEntry] {
        return [
            LocalizedEntry(
                ...
                • Rétablissement automatique : pour les déclencheurs de faible confiance (comme ClickFix), le filet de sécurité de l'assistant rétablit le réseau en 10 minutes même sans levée. Pour ceux de forte confiance (leurre de rançongiciel, détection XProtect), le réseau ne se rouvre pas de lui-même : après 1 heure au plus, on passe en mode dégradé jusqu'à ce que vous leviez depuis la fenêtre d'urgence ou la notification, ou qu'une nouvelle vérification confirme que la cause a disparu. Cela vaut aussi après une fermeture, un plantage ou un redémarrage.
